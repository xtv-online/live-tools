...
module.exports.getEachRoleWithConnectedClients = function() {
    var deferred = Q.defer();

    module.exports.getRoles()

    .then(function (roles) {
        return [roles, module.exports.getConnectedClients()];
    })

    .spread(function (roles, clients) {
        for (var ai = 0; ai < roles.length; ai++) {
            var identifiedClients = [];
            var roleID = roles[ai]._id;

            for (var bi = 0; bi < clients.length; bi++) {
                var clientRoleID = clients[bi].role[0]._id;

                if (roleID === clientRoleID) {
                    newClient = {
                        '_id' : clients[bi]._id,
                        'isWebRtcCapable' : clients[bi].isWebRtcCapable
                    };
                    identifiedClients.push(newClient);
                }
            }

            roles[ai].clients = identifiedClients;
        }
    ...
    .catch(function (error) {
        console.error('Database Error'.red.bold, error);
        deferred.reject(new Error(error));
    });
